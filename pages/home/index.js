...
function criarCardDeVaga(vaga){
    let id         = vaga.id
    let titulo     = vaga.title
    let empresa    = vaga.enterprise
    let cidade     = vaga.location
    let descricao  = vaga.descrition
    let modalidade = vaga.modalities
...
function adicionarNaLista (lista, referenciaHtml){
    ...
    lista.forEach(element => {
        ...
        let vagaCriado = criarCardDeVaga (element)
        referenciaHtml.appendChild(vagaCriado)
    })

}

adicionarNaLista (jobsData, containerReceberVagas)

let selecionarVaga = []

function criarCardSelecionado (vaga){
    const listaVagasSeslecionadas = document.querySelector(".conteiner_receber_vagas_selecionadas")
    ...
    tagBtLixeira.addEventListener("click", () =>{
        
        const elementoEncontrado = selecionarVaga.findIndex((element) => element)
    
        selecionarVaga.splice(elementoEncontrado,1)
    ...
    listaVagasSeslecionadas.appendChild(tagLi)
    
}

function armazenarDados(){
    const vagaLocalJoson = localStorage.getItem(selecionarVaga)
    if(vagaLocalJoson){
        const vagaLocal = JSON.parse(vagaLocalJoson)
        return selecionarVaga(vagaLocal)
    }
}
armazenarDados()
...
function selecionarCandidatura (){
    ...
    const botao = botaoCandidatar.forEach((botaoAtual) =>{
        ...
        botaoAtual.addEventListener("click", (event) =>{
            if(botaoAtual.innerText === "Candidatar"){

                event.preventDefault()
                let elemento = event.target
                let idElemento = elemento.id
                let id = parseInt(idElemento)
                let produtoEncontrado = jobsData.find((element)=>{
                        return element.id == id
                })

                if(produtoEncontrado){

                criarCardSelecionado(produtoEncontrado)
                selecionarVaga.push(produtoEncontrado)
            }
            }

            const vagasJson = JSON.stringify(selecionarVaga)
            localStorage.setItem("vagas", vagasJson)
